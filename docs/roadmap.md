...
本人指示で、当日の流れを「粗く作る（安いモデル）→研究（賢いモデル）→計画（賢いモデル）→ゼロから作り直す（安いモデル）」の4段階・8ステップへ変更。正本は `kit/docs/guide-steps.md`。前提として開発ログ（`docs/log.md`・`dev-log`）とモデル切替前のファイル書き出しを全段階に通した。モデルの使い分けは `ai-setup` とキットの `.claude/settings.json`（Codexはフォルダ単位の設定が効かないため段階ごとに画面で切替。2026-09-25実測）。
