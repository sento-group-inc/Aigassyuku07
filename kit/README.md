...
Step 5（段階4 ゼロから作り直す）で、ログイン画面つきのテンプレートからアプリを作り、このフォルダの中身をコピーします。手順は [dashboard-repo](.claude/skills/dashboard-repo/SKILL.md) にあります。ガイドに「次は何をすればいい？」と聞けば、その時点で案内されます。
